...
var Question = {
    ...
    , format: function (q) {
        ...
        if (q.answer_count == 0) template.find('.answers').addClass('unanswered');
        if (q.is_accepted) template.find('.answers').addClass('accepted');
        ...
        if (!q.autoupdate) template.find('.actions .monitor').addClass('off').attr('title', 'Not monitoring. Click to enable monitoring.');
    ...
    , getIdFromInput: function (url) {
        if (isNumeric(url)) return url; //if an id is entered, just return that
        var bits = url.split('/');
        if (isNumeric(bits[4])) return bits[4]; //where the id generally is
        else {
    ...
    , getQuestionsFromApi: function (ids, callback, site) {
        $.ajax({
		    ...
		    , success: function (data) { if (typeof callback == "function") callback(data, site); }
		    , error: function (err) {
		        var data = {
		            "error": err
    ...
    , handleError: function (data) {
        if (!isIncremented) { //if not working, increment by .1 and try again. still not working, default to 1.0
            ...
            isIncremented = true;
            Api.version = (parseFloat(Api.version) + .1) + ''; //increment then re-stringify
            ...
            try {
                ...
                BG.getPopup();
                if (BG.popup) BG.popup.Form.handleError(msg);
            }
    ...
    , removeAll: function () {
        for (var id in localStorage) {
            if (id.indexOf('|') != -1) Question.remove(id);
        }
    }
